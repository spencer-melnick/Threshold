...
/*
 * Class used as the base for any inventory item type. Most classes should probably inherit from UInventoryItemType
 * instead of this.
 */
UCLASS(Abstract)
class INVENTORYSYSTEM_API UInventoryItemTypeBase : public UPrimaryDataAsset
{
	GENERATED_BODY()
...
public:
	...
	*/
	 ...
	 */
	virtual bool operator==(const UInventoryItemTypeBase& OtherType) const { return GetClass() == OtherType.GetClass(); }

	bool operator!=(const UInventoryItemTypeBase& OtherType) const { return !operator==(OtherType); }
