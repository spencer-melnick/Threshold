...
#include "AT_ApplySlowdownCurve.h"
...
void UAT_ApplySlowdownCurve::Activate()
{
	...
	{
		UE_LOG(LogThresholdGeneral, Warning, TEXT("UAT_ApplySlowdownCurve called by %s with no AbilitySystemComponent in instance %s"),
	...
	if (!SlowdownCurve)
	{
		UE_LOG(LogThresholdGeneral, Warning, TEXT("UAT_ApplySlowdownCurve called by %s with no SlowdownCurve in instance %s"),
            *GetNameSafe(Ability), *InstanceName.ToString())
		return;
	}
