...
#include "ThresholdGame/Abilities/AttributeSets/BaseAttributeSet.h"
...
void ABaseCharacter::AttachTargetIndicator(AActor* TargetIndicator)
{
	// A sanity check to make sure there actually is a target indicator - only check on development builds
		...
		if (!TargetIndicator)
		{
			UE_LOG(LogThresholdGame, Error, TEXT("Attempting to attach null TargetIndicator to character %s"),
				*GetNameSafe(this))
			return;
		}
	#endif
	...
	{
		UE_LOG(LogThresholdGame, Warning, TEXT("Attaching target indicator %s to character %s - character has no "
	...
	if (bUseSocketForTargetLocation)
	{
		...
		UE_LOG(LogThresholdGame, Warning, TEXT("Attaching target indicator %s to character %s - socket %s is invalid "
...
const USkeletalMeshSocket* ABaseCharacter::GetTargetSocket() const
{
	...
	if (!SkeletalMeshComponent)
	{
		UE_LOG(LogThresholdGame, Warning, TEXT("Character %s has bUseSocketForTargetLocation  = true "
...
{
	// Do something
	UE_LOG(LogThresholdGame, Display, TEXT("%s died"), *GetNameSafe(this))
