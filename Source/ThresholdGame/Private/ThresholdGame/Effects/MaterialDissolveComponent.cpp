...
#include "ThresholdGame/Effects/MaterialDissolveComponent.h"
...
void UMaterialDissolveComponent::StartDissolve()
{
    if (bIsDissolving)
    {
        UE_LOG(LogThresholdGame, Warning, TEXT("Dissolve Component %s is already dissolving, cannot trigger another dissolve"),
            *GetNameSafe(this));
        return;
    }
    ...
    {
        UE_LOG(LogThresholdGame, Error, TEXT("Dissolve Component %s does not have a valid target MeshComponent"),
            *GetNameSafe(this));
    }
    ...
    for (int32 i = 0; i < DissolveMaterials.Num(); i++)
    {
        ...
        if (MaterialInstanceDynamic == nullptr)
        {
            ...
            UE_LOG(LogThresholdGame, Error, TEXT("Failed to create dynamic instance of material %s"),
